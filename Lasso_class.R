...
model_full <- train(x = X, y = Y,
                    method = "glmnet",
                    metric = "ROC",
                    tuneGrid = expand.grid(alpha = 1, lambda = exp(seq(-6, 1, length = 100))),
                    trControl = ctrl)
...
for (n in feature_counts) {
  ...
  model_n <- train(x = X[, vars_n], y = Y,
                   method = "glmnet",
                   metric = "ROC",
                   tuneGrid = expand.grid(alpha = 1, lambda = model_full$bestTune$lambda),
                   trControl = ctrl)
...
model_Lasso <- train(x = X[, top_features_lasso], y = Y,
                     method = "glmnet",
                     metric = "ROC",
                     tuneGrid = expand.grid(alpha = 1, lambda = model_full$bestTune$lambda),
                     trControl = ctrl)
...
pred <- predict(model_Lasso, newdata = X_test)
probs <- predict(model_Lasso, newdata = X_test, type = "prob")[, "S"]
...
coef_df <- data.frame(Variable = rownames(coef_lasso_final)[-1],
                      Coef = coef_lasso_final[-1, 1])
